...
def evaluateSingleModel(model, config, eps):
    result = []
    
    for n_repl in range(config['S_obs']):
        data = model.rvs(config['n_samples'], eps=eps, dof=config['dof'])
...
def evaluateSetOfModels(config):
    ...
    for eps in np.linspace(0, 1, 10):
        for n_model in range(config['S_sg']):
            model = TensorGraphicalModel(config['dims'], config['densities'])

            single_results = evaluateSingleModel(model, config, eps)
            single_results['eps'] = eps
